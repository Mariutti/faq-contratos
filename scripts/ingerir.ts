...
function extrairEsperaSugeridaMs(erro: any): number | null {
  let corpo: any;
  try {
    corpo = JSON.parse(erro?.message ?? "");
  } catch {
    return null; // a mensagem não era um JSON -- não tem como saber
  }

  const detalhes = corpo?.error?.details;
  if (!Array.isArray(detalhes)) return null;

  const retryInfo = detalhes.find((d: any) =>
    String(d?.["@type"] ?? "").includes("RetryInfo")
  );
  const bruto = retryInfo?.retryDelay; // formato tipo "58s"
...
async function comRetentativa<T>(fn: () => Promise<T>, tentativas = 3): Promise<T> {
  for (let tentativa = 1; tentativa <= tentativas; tentativa++) {
    try {
      ...
      const ehLimiteDeCota = erro?.status === 429;
...
async function processarPdf(nomeArquivo: string) {
  ...
  if (status === "completo") {
    console.log(`  ${nomeArquivo}: já processado (${trechos.length} trecho(s)) -- pulando`);
    return;
  }
...
async function main() {
  if (!fs.existsSync(PASTA_PDFS)) {
    ...
    console.log(`Crie a pasta "documentos" na raiz do projeto e coloque os PDFs lá.`);
    return;
  }
